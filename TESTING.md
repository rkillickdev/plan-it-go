...
When I investigated the 'violating node', it is related to labels created by Google maps.  I mentioned this to my mentor and decided that this did not need to be addressed for now, as it is caused by the rendering of the map.
...
| static/js/img-src-backups.js | Navigate to a page where the src for an image is served from an external site and an error is returned | Placeholder url injected into src for the image with a broken link | See Known Bugs |
| static/js/map-cluster.js | Navigate to trip detail page | Google Map for the specified trip location displayed with markers identifying each place that has been added to the trip itinerary.  If places are close together they are represented as a cluster. | Pass |
...
I passed my JavaScript files through the linter [JSHint](https://jshint.com/) to validate and check for any syntax errors.  Reports for each file can be viewed below.
...
* The toasts.js file notes one undefined variable but it is needed to implement this feature, so I have not addressed the warning.
...
* I fixed these by moving the outer scoped variables inside the function.  These js files are now clear of warnings.
...
All pages have been run through the [W3C](https://validator.w3.org/) Markup Validator.  Initially a couple of recurring errors appeared in various templates.  These are outlined below and have now been resolved:
...
* I was initially triggering modals using an anchor link, but Lighthouse alerted me to the fact that these links were [not crawlable](https://developers.google.com/search/docs/crawling-indexing/links-crawlable?visit_id=638351626897775578-1280043289&rd=1). I resolved this by using a button element to trigger these modal instead.

### Opportunities

Several opportunities were flagged by the lighthouse report to improve performance particularly when testing for mobile:
...
The images referred to in this situation are mostly served by the trip advisor cdn, so I have not been able to alter the size or format of these files.  Having tested the site on various devices and networks, the issue does not seem to be impacting page load times to the detriment of the user.
