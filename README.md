...
A [Product Backlog](https://github.com/rkillickdev/plan-it-go/milestone/1) milestone was created on GitHub to establish a 'single authoritative source of work'.  I initially moved all user stories to the backlog before they were allocated to sprints.  The product backlog was refined throughout the course of development with user stories reprioritised as the project evolved.
...
Django allauth has been used to implement user registration and authentication. Certain functionality for the site can only be accessed once a user is authenticated.  The default Django allauth templates used for signing up, logging in and logging out have been styled to match the rest of the site and ensure a consistent user experience.
...
On some views, if statements have been used along side `raise PermissionDenied()` to ensure the 403 error template is displayed when permissions of the logged in user are not valid for the request.
...
| I tried to add a success message on the image upload page using the SuccessMessageMixin but the message was not displaying.  I have therefore removed this code as it was not providing the expected functionality.  For now I do not believe it has too much of a negative impact on the user experience. Feedback is already provided to the user that their image has been uploaded successfully as it displays in their gallery on the page as soon as upload is complete | I am using this method successfully in other views.  My ImageUpload view is slightly more complex in terms of the success URL code as I want the user to remain on the same page once an image has been uploaded, so I believe this could be causing the  success message not to display as expected. With the benefit of more time, I plan to find a solution for this in future sprints. |

<br>

## **Solved Bugs**

Below are bugs that were discovered during development and testing.  I have not listed some of the smaller quick fixes that were implemented during the testing phase, but they were raised as 'bug' issues on GitHub and can be seen as part of my [Project Kanban Board](https://github.com/users/rkillickdev/projects/4).
...
| During testing, I discovered that users might create a trip name that was unique, but when 'slugified' it would not be unique.  For example there could be two trip names 'LA LA Land' and 'la la land'.  When an instance of Trip is created and saved to the database, the slug field is auto populated and both would be 'slugified' as 'la-la-land'.  This throws an error and directs the user to the error 500 template. | I removed `unique=True` from the slug field in Trip model and Location model. This does not cause any further issues, as I am only using the slug field as an informational element in URLs therefore it is not a problem if two slugs are identical. The slug is always accompanied by the primary key, and this is the field that is used to look up records from the database. View [GitHub Issue#68](https://github.com/rkillickdev/plan-it-go/issues/68) to see the full steps taken to resolve |
...
* [Referenced Google Docs about Maps API](https://developers.google.com/maps)
